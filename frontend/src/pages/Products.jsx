...
const Products = () => {
  ...
  const handleSave = async () => {
    try {
      const res = await updateProduct(editedProduct._id, editedProduct);

      setProducts((prevProducts) =>
        prevProducts.map((product) =>
          product._id === editedProduct._id ? res.data : product,
        ),
      );
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesSearch = productName.includes(search) || sku.includes(search);
  ...
  const stats = {
    ...
    inStock: filteredProducts.filter((p) => getProductStatus(p) === "In Stock")
      .length,
  ...
  return (
    <div>
      ...
      <WarehouseInventoryTable products={products} setProducts={setProducts} />
    </div>
  );
};
